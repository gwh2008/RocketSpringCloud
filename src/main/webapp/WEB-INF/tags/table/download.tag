...
<button class="btn btn-white  btn-sm" data-toggle="tooltip" data-placement="left" onclick="download()" title="下载模块"><i class="fa fa-file-text-o"></i> ${label==null?'下载模块':label}</button>
...
<script type="text/javascript">
    ...
    function download(){
        ...
        top.layer.confirm('确认要下载模块吗?moduleName: '+moduleName, {icon: 3, title:'系统提示'}, function(index){
            //do something
            $.ajax({
                type : "POST",
                url : "${ctx}/gaowh/gaowh/download",
                data : {"moduleName":moduleName},
                //dataType : "JSON",
                success : function(data, textStatus, jqXHR) {
                    top.layer.msg('模块下载成功！', {
                        icon: 1,
